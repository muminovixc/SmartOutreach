...
export default function AIComposerModal({
  ...
  const sendEmailViaGmail = async () => {
    // 1. Određujemo prioritet: prvo AI pronađeni email, pa tek onda email iz baze
    const targetEmail = foundEmail || lead.email;

    // 2. Provjera da li email uopšte postoji
    if (!targetEmail || targetEmail === "no email" || targetEmail === "") {
      return alert(
        "Nije pronađena email adresa primaoca. Molimo unesite je ručno ili provjerite sajt."
      );
    }
    ...
    const token = localStorage.getItem("token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/campaigns/send`,
        {
          ...
          headers: {
            ...
            // Obavezno šaljemo token jer je ovo zaštićena ruta na backendu
            "Authorization": `Bearer ${token}`,
          },
          body: JSON.stringify({
            target_email: targetEmail,
            lead_name: lead.title, // Šaljemo ime leada za kampanju u bazi
      ...
      if (res.ok) {
        ...
        onClose(); // Zatvaramo modal nakon slanja
      } else {
        const errorData = await res.json();
        alert(`Greška pri slanju: ${errorData.detail || "Nepoznata greška"}`);
      }
    } catch (err) {
      console.error("Slanje maila propalo:", err);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-sm bg-black/80">
      <motion.div
        ...
        className="relative w-full max-w-xl bg-[#0c0e12] border border-zinc-800 rounded-[2.5rem] p-8 shadow-2xl"
      >
        <button
          onClick={onClose}
        ...
        <div className="mb-6">
          ...
          <div className="mt-3 space-y-2">
            <input
              ...
              className="w-full bg-zinc-950/50 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-[#00F5D4] outline-none focus:border-[#00F5D4]/50 transition-all"
            />
              ...
              {(foundEmail || lead.email) &&
                ...
                lead.email !== "no email" && (
                  <span className="text-[#00F5D4] text-[9px] font-bold uppercase px-2 py-0.5 bg-[#00F5D4]/10 rounded-full border border-[#00F5D4]/20 flex items-center gap-1">
                    <Mail size={10} /> {foundEmail || lead.email}
                  </span>
                )}
            </div>
          </div>
        ...
        {generationStep === "form" && (
          <div className="space-y-5">
            ...
            <textarea
              ...
              className="w-full h-32 bg-zinc-950 border border-zinc-800 rounded-2xl px-4 py-4 text-sm text-white resize-none"
            />
            <button
              onClick={generateAIEmail}
              className="w-full py-4 bg-[#00F5D4] text-black font-bold uppercase font-['Syne'] rounded-2xl flex items-center justify-center gap-2"
            >
        ...
        {generationStep === "result" && (
          <div className="space-y-4">
            <input
              ...
              className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-[#00F5D4]"
            />
            <textarea
              value={editableBody}
              onChange={(e) => setEditableBody(e.target.value)}
              className="w-full h-64 bg-zinc-950 border border-zinc-800 rounded-2xl px-4 py-4 text-sm text-zinc-300 leading-relaxed resize-none"
            />
            <div className="flex gap-3 pt-2">
              <button
                onClick={() => setGenerationStep("form")}
                className="flex-1 py-4 border border-zinc-800 rounded-xl text-zinc-500 text-[10px] font-bold uppercase"
              >
                Reset
              </button>
              <button
                onClick={sendEmailViaGmail}
                disabled={isSending || (!foundEmail && !lead.email)}
                className={`flex-[2] py-4 rounded-xl flex items-center justify-center gap-2 font-bold uppercase text-[10px] transition-all 
                ${
                  !foundEmail && !lead.email
                    ? "bg-zinc-800 text-zinc-500 cursor-not-allowed"
                    : "bg-gradient-to-r from-[#00F5D4] to-[#00A8FF] text-black hover:opacity-90 active:scale-95"
                }`}
              >
                {isSending ? (
                  ...
                  <>
                    <Send size={14} />
                    {!foundEmail && !lead.email ? "Missing Email" : "Send Now"}
                  </>
                )}
              </button>
            </div>
            <button
              ...
              className="w-full text-[9px] uppercase text-zinc-600 hover:text-[#00F5D4] flex items-center justify-center gap-2"
            >
