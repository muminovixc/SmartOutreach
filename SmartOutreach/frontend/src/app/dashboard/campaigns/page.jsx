...
import {
  Mail, Calendar, User, ExternalLink, Search, CheckCircle2,
  Loader2, RefreshCcw, Sparkles, TrendingUp, ChevronDown, MessageSquare, Filter
} from "lucide-react";
...
export default function CampaignsPage() {
  ...
  const fetchCampaigns = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/campaigns/history?t=${Date.now()}`, {
  ...
  const handleSync = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/campaigns/sync-all`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) setTimeout(() => { fetchCampaigns(); setSyncing(false); }, 3000);
  ...
  const fetchThreadMessages = async (campaignId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/campaigns/${campaignId}/thread`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const toggleExpand = (id) => {
    ...
    else { setExpandedId(id); fetchThreadMessages(id); }
  };
      ...
      <div className="flex-1 max-w-6xl mx-auto ml-20 md:ml-64">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={handleSync} disabled={syncing} className="p-4 bg-zinc-900 border border-zinc-800 rounded-2xl hover:bg-zinc-800 transition-colors">
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="text-center py-20 text-zinc-600 uppercase font-bold text-xs tracking-widest border border-dashed border-zinc-800 rounded-3xl italic">
              No results found for "{searchTerm}" with status "{filterStatus}"
            </div>
          ) : (
                        ...
                        {threadLoading ? (
                          ...
                          currentThread.map((msg, idx) => (
                            <div key={idx} className={`flex w-full ${msg.is_me ? "justify-end" : "justify-start"}`}>
                              <div className={`max-w-[85%] p-5 rounded-3xl ${msg.is_me ? "bg-[#00F5D4]/10 border border-[#00F5D4]/20 rounded-tr-none shadow-[0_4px_20px_rgba(0,0,0,0.3)]" : "bg-zinc-900 border border-zinc-800 rounded-tl-none shadow-xl"}`}>
